<template>
  <div class="relative slyme-glass rounded-2xl overflow-hidden shadow-sm flex flex-col">
    ...
    <div
      class="bg-gray-100/80 dark:bg-gray-800/80 px-4 py-3 border-b border-gray-200/60 dark:border-gray-700/60 flex items-center justify-between z-10">
      ...
      <span v-if="filename"
        class="font-mono text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest font-semibold">
        {{ filename }}
      </span>

      <div class="w-16 flex justify-end">
        <button v-if="code" @click="handleCopy"
          class="text-gray-400 dark:text-gray-500 hover:text-black dark:hover:text-white transition-colors focus:outline-none"
          :title="copied ? 'Copied!' : 'Copy code'">
          <svg v-if="copied" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"
            viewBox="0 0 24 24" class="text-green-500 dark:text-green-400">
    ...
    <div
      class="p-6 overflow-x-auto bg-white/60 dark:bg-black/40 text-sm font-mono leading-relaxed relative shiki-container">
      <slot></slot>
    </div>
  </div>
</template>
