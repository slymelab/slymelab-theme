  ...
  <div class="slyme-glass slyme-glass-hover group relative flex flex-col h-full p-8 rounded-[1.5rem] overflow-hidden">
    ...
    <div
      ...
      <div class="flex justify-between items-start mb-8">
        <div
          class="font-mono text-[10px] tracking-widest uppercase bg-gray-200/50 px-3 py-1.5 rounded-md border border-gray-300/30 font-semibold">
          <slot name="badge">PRODUCT</slot>
        </div>
        <div class="transition-colors duration-300">
          <slot name="icon">
            <svg v-if="isExternalLink" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="mt-10 relative w-fit font-mono text-sm font-medium flex items-center gap-2 group-hover:gap-3 transition-all duration-300">
        <slot name="action">Learn More -></slot>
        <span v-if="showUnderline" class="absolute left-0 -bottom-1 h-[1.5px] w-0 bg-current transition-all duration-300 group-hover:w-full"></span>
      </div>
    </div>
  </div>
</template>
